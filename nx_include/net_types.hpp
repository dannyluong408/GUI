...
#define C_UNION union
#endif

#define GAME_VERSION 314
#define KEYBIND_VERSION 1
#define NETWORK_DEFAULT_PORT 3224

#define NETWORK_PING_TIMEOUT 5000 // milliseconds
...
// These might eventually be split
...
union __attribute__ ((__packed__)) target_t {
	C_STRUCT target_entity t;
	C_STRUCT vec3f position;
};
...
struct __attribute__ ((__packed__)) change_target_event {
	C_STRUCT target_entity entity, target;
};
...
struct __attribute__ ((__packed__)) server_move_event {
	C_STRUCT movement_state ms;
	C_STRUCT target_entity entity;
};
...
// CP_AUTHME
struct __attribute__ ((__packed__)) network_login {
...
struct __attribute__ ((__packed__)) network_confirm_login {
	...
	C_STRUCT network_player_data player_data;
...
struct __attribute__ ((__packed__)) health_resource_state {
	uint16_t health, health_max, resource, resource_max;
	C_STRUCT precise_time_t time;
	C_STRUCT target_entity target;
};

// SP_UPDATE_BUFF
struct __attribute__ ((__packed__)) network_buff {
	C_STRUCT target_entity target;		// entity id of the target
	...
	C_STRUCT precise_time_t origin_time;
};
...
struct __attribute__ ((__packed__)) network_damage {
	C_STRUCT target_entity target, damager;
	C_STRUCT precise_time_t time;
...
struct __attribute__ ((__packed__)) spell_cast_event {
	...
	// Necessary?
	//int32_t value;
	C_UNION target_t target;
	C_STRUCT target_entity caster;
};
...
struct __attribute__ ((__packed__)) interrupt_t {
	C_STRUCT target_entity interrupted, interrupter;
	SPELL_ID spell_id;
	// In milliseconds
...
struct __attribute__ ((__packed__)) client_target {
	C_STRUCT target_entity new_target;
};
...
struct __attribute__ ((__packed__)) world_settings_t {
	...
	C_STRUCT movement_state new_state;
};
...
struct __attribute__ ((__packed__)) cooldown_state_ext {
	PLAYER_ID player;
	C_STRUCT cooldown_state_t cd_state;
};
...
struct __attribute__ ((__packed__)) keybind_set_t {
	...
	C_STRUCT keybind_key action_bar[5][10],
		// Movement
